...
def create_ai_bottleneck_analyst(llm):
    ...
    def ai_bottleneck_analyst_node(state):
        ...
        system_message = (
            ...
            "\n"
            "  - Prefer entries on pullbacks: RSI 40–60 near the 50-SMA beats"
            "    RSI > 75 chasing. If RSI > 80, wait — flag 'extended, buy the"
            "    next 50-SMA touch'."
            "\n\n"
            ...
            "\n"
            "  - Size: risk 1% of portfolio to the stop. Stop = 2.5 × ATR below"
            "    entry. size = (portfolio × 0.01) / (2.5 × ATR). Assume a $100,000"
            "    portfolio if none is given. STRONG momentum + bottleneck score ≥ 8"
            "    may size up to 1.5% risk."
            "\n"
            "  - Max single-name exposure: 10% of portfolio. Max per bottleneck"
            "    category: 20% (owning three cooling names is one bet, not three)."
            "\n"
            "  - Exits, whichever hits first: close below 200-SMA; 25% trailing"
            "    stop from the highest close since entry; or thesis break from"
            "    Step 4."
            "\n\n"

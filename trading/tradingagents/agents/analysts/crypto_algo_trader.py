...
def create_crypto_algo_trader(llm):
    ...
    def crypto_algo_trader_node(state):
        ...
        system_message = (
            ...
            "  - −5 to −6: STRONG SELL"
            "\n\n"
            "**Position sizing — N-based Turtle units (walk-forward validated"
            " on BTC/ETH 2019–2026; crypto's fat tails punish classic 4-unit"
            " pyramids):**"
            "\n"
            "  N = ATR(20). 1 Unit = (portfolio_value × 0.01) / (N × current_price)"
            "\n"
            "  - STRONG BUY/SELL (System 2 confirmed): up to 2 Units"
            "\n"
            "  - BUY/SELL (System 2 or strong System 1): up to 2 Units"
            "\n"
            "  - WEAK BUY/SELL (System 1 only): 1 Unit"
            "\n"
            "  - Pyramid rule: add 1 Unit per 1.0N favorable move, max 2 Units total"
            "\n"
            "  - Hard stop: 1.5N from the last entry price (validated vs 2N/2.5N)"
            "\n"
            "  - The ADX < 20 veto is backtest-confirmed: removing it cut BTC"
            "    MAR from 0.92 to 0.71 — never trade a breakout in a dead-ADX"
            "    regime"
            "\n\n"

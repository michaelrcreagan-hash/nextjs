...
export default function CryptoDerivs() {
  ...
  const ca = data.source === 'coinapi'
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="text-sm text-slate-400 flex items-center gap-2">
          ⚡ Perps &amp; Futures · funding + open interest
          <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${ca ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-700 text-slate-400'}`}>
            {ca ? 'COINAPI (cross-exchange)' : 'BINANCE'}
          </span>
        </div>
      ...
      <div className="text-[11px] text-slate-600 bg-slate-900/40 border border-slate-800 rounded-lg p-3">
        {ca
          ? 'Funding rate & open interest are live from CoinAPI derivatives metrics (DERIVATIVES_FUNDING_RATE_CURRENT / DERIVATIVES_OPEN_INTEREST on Binance USDⓈ-M), priced via CoinGecko — US-accessible. For visual liquidation clusters use the CoinGlass heatmap link and feed levels into the Altcoin Squeeze T1 targets.'
          : 'Funding & OI are from Binance USDⓈ-M futures (fallback — may be geo-restricted). Set COINAPI_KEY to use CoinAPI cross-exchange metrics instead.'}
      </div>
    </div>
  )
}

...
function generateOptionsChain(spot: number, symbol: 'IBIT' | 'MSTR' | 'VIX'): OptionStrike[] {
  ...
  for (let i = -range / steps; i <= range / steps; i++) {
    ...
    const putSkew = strike < spot ? 1.3 : 0.85 // more puts OTM (fear)
...
function calcMaxPain(chain: OptionStrike[]): number {
  ...
  for (const row of chain) {
    ...
    for (const r2 of chain) {
      // call writers lose: max(0, spot - strike) × OI
      if (row.strike > r2.strike) pain += (row.strike - r2.strike) * r2.callOI
      // put writers lose: max(0, strike - spot) × OI
...
const SPOT: Record<Sym, number> = { IBIT: 38, MSTR: 315, VIX: 20 }
...
export default function OptionsFlow() {
  ...
  const spot = activeSymbol === 'VIX' ? vixLevel : SPOT[activeSymbol]
  const chain = generateOptionsChain(spot, activeSymbol)
  ...
  const ivSkewData = chain.slice(Math.max(0, chain.findIndex(c => c.strike >= spot * 0.85)), chain.findIndex(c => c.strike >= spot * 1.15) + 1)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap gap-3 items-center">
        ...
        <div className="text-xs text-slate-500 ml-2">Spot: ${spot.toFixed(activeSymbol === 'IBIT' ? 2 : activeSymbol === 'VIX' ? 2 : 0)} · Mock data (replace with broker API)</div>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          { label: 'VIX Level', val: vixLevel.toFixed(2), note: vixLevel < 18 ? 'Low fear — bearish for shorts' : vixLevel > 30 ? 'High fear — shorts working' : 'Elevated', color: vixLevel > 25 ? '#ef4444' : '#10b981' },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={chain.slice(Math.max(0, chain.findIndex(c => c.strike >= spot * 0.9)), chain.findIndex(c => c.strike >= spot * 1.1) + 1)} barGap={0}>
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
          ...
          <div className="text-xs text-slate-500 mb-3">+GEX = dealers long gamma (suppresses vol) · -GEX = dealers short gamma (amplifies vol)</div>
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={gexData.slice(Math.max(0, gexData.findIndex(d => d.strike >= spot * 0.92)), gexData.findIndex(d => d.strike >= spot * 1.08) + 1)}>
              <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
              <XAxis dataKey="strike" tick={{ fill: '#64748b', fontSize: 10 }} tickFormatter={v => `$${v}`} />
              ...
              <Bar dataKey="gex" radius={[2, 2, 0, 0]}>
                {gexData.slice(Math.max(0, gexData.findIndex(d => d.strike >= spot * 0.92)), gexData.findIndex(d => d.strike >= spot * 1.08) + 1).map((d, i) => (
        ...
        <div className="bg-slate-800/60 border border-slate-700 rounded-xl p-4">
          ...
          <div className="mt-3 text-xs text-slate-500">
            ⚠️ Options data is simulated. For live data, connect Interactive Brokers, Tastytrade, or Deribit API.
          </div>
        </div>
      </div>
    </div>

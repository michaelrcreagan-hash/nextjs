...
| Earnings estimate revisions | 30/90-day | AlphaVantage `EARNINGS_ESTIMATES` exists and covers this directly, but hit its **25-requests/day** free-tier cap on the first call this session. **Backup path confirmed via WebSearch**: real NVDA pull found EPS estimate revised +6.69% (next-quarter revenue) and +3.08% (Zacks consensus, past month), analyst buy-consensus 85/100 across 68 analysts, most recent 3 rating changes dated and named (China Renaissance, Needham, DA Davidson) | ⚠️ AlphaVantage quota-limited; WebSearch works per-ticker |
| Scarcity/moat data (lead times, backlog) | Qualitative/quarterly | **Confirmed via WebSearch**: real MU pull found HBM 2026 supply fully sold out/contracted, industry-wide shortage seen persisting to 2028 per Intel's CEO, HBM demand +70% YoY 2026, $25B fiscal-2026 capex tied to locked-in HBM demand — genuinely usable as a manual per-ticker scarcity score input | ✅ Reachable via WebSearch (manual, per-ticker) |
| Hyperscaler capex guidance | Quarterly | **Confirmed via WebSearch**: real combined 2026 guidance found — $725B total (+77% YoY), Microsoft ~$190B, Amazon ~$200B, Alphabet $175-185B, Meta $125-145B, plus latest-quarter actuals (Microsoft $30.88B Q3 capex +84% YoY, Amazon $44.2B, Alphabet $35.67B) | ✅ Reachable via WebSearch |
| Analyst upgrades | 30-day net | Same NVDA WebSearch pull surfaced dated, named analyst rating changes — usable as a manual per-ticker count, not a bulk feed | ✅ Reachable via WebSearch (manual, per-ticker) |
| **Finnhub connector** | — | User asked to add Finnhub — **not showing up as a connected tool this session**; adding a new connector isn't something I can do from here. Needs the user to add it via claude.ai connector settings, or `claude mcp add` in an interactive CLI session | ❌ Not connected, needs user action |
...
**WebSearch caveat:** genuinely resolves the earnings-revision, scarcity/moat, capex-guidance, and analyst-upgrade gaps with real, cited data — but it's a one-ticker-at-a-time manual research action, not a batch API. There's no automated way to run it inside `src/features.py` at backtest time; it fits the **top-3/weekly** cadence (a human or agent runs a few searches per name, weekly) rather than a daily 52-ticker sweep. Reflected in `config.yaml` → `api_scan_policy` below.

**Scan throttle policy (user directive):** the Tipranks (options-flow, hedge-fund activity, investor sentiment), AlphaVantage (earnings estimates), and WebSearch (earnings revisions, scarcity/moat, capex guidance, analyst upgrades) scans all run against the **top 3 stocks only, once a week** — re-ranked by Phase Score each run, not a fixed list. See `config.yaml` → `api_scan_policy`.
